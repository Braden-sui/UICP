...
describe('stream cancellation', () => {
  ...
  it('normal flush returns cancelled false', async () => {
    const onBatchMock = vi.fn().mockResolvedValue({ success: true, applied: 1, errors: [], skippedDuplicates: 0, deniedByPolicy: 0, batchId: '' } as ApplyOutcome);
    const agg = createOllamaAggregator(onBatchMock);
  ...
  it('soak test: rapid start/stop with timing assertions', async () => {
    ...
    const onBatchMock = vi.fn(async (batch: Batch) => {
      ...
      return { success: true, applied: batch.length, errors: [], skippedDuplicates: 0, deniedByPolicy: 0, batchId: '' } as ApplyOutcome;
    });
