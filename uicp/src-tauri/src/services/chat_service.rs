...
pub async fn stream_chat_completion(
    ...
    // Convert to JSON to avoid cross-module private field access
    let req_value = serde_json::to_value(&request).map_err(|e| e.to_string())?;
    let messages = req_value
        .get("messages")
        .cloned()
    ...
    let requested_model = req_value
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_else(|| "qwen3-coder:480b".into());
    ...
    let mut body = serde_json::json!({
        ...
        "messages": messages,
        "stream": req_value.get("stream").and_then(|v| v.as_bool()).unwrap_or(true),
    });
    if let Some(tools_val) = req_value.get("tools").cloned() {
        body["tools"] = tools_val;
    }
    if let Some(format_val) = req_value.get("format").cloned() {
        body["format"] = format_val;
    }
    if let Some(response_format_val) = req_value.get("response_format").cloned() {
        body["response_format"] = response_format_val;
    }
    if let Some(tool_choice_val) = req_value.get("tool_choice").cloned() {
        body["tool_choice"] = tool_choice_val;
    }
    if let Some(reasoning_val) = req_value.get("reasoning").cloned() {
        body["reasoning"] = reasoning_val.clone();
        if use_cloud {
            if let Some(options_val) = req_value.get("options").cloned() {
    ...
    let body = adapter.transform_request(body).await?;

    // Determine base URL: allow frontend override when explicitly permitted.
    let allow_override = std::env::var("UICP_ALLOW_BASE_URL_OVERRIDE")
        .ok()
    ...
    let override_used_for_task = override_used;
    ...
    let join: JoinHandle<()> = spawn(async move {
        ...
        if debug_on {
            if override_used_for_task {
                let ev = serde_json::json!({
                    "ts": Utc::now().timestamp_millis(),
                    "event": "base_url_override_used",
            ...
            let ev = serde_json::json!({
                ...
                "baseUrlOverrideUsed": override_used_for_task,
            });
            tokio::spawn(append_trace(ev.clone()));
            tokio::spawn(emit_debug(ev));
...
/// Probe local Ollama daemon and toggle `use_direct_cloud` accordingly when local opt-in is enabled.
/// INVARIANT: Will not enable local unless `allow_local_opt_in` is true.
pub async fn maybe_enable_local_ollama(state: &AppState) {
    ...
    let url = format!("{}/models", OLLAMA_LOCAL_BASE_DEFAULT);
