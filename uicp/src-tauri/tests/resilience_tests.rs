...
async fn test_retry_policy_categories() {
    ...
    assert!(delay.unwrap() >= Duration::from_millis(2000), "OpenAI rate limit delay should be at least 2s");
...
async fn test_retry_attempts_exhausted() {
    ...
    assert!(delay.is_none(), "Sixth attempt should not retry (exhausted)");
}
...
async fn test_provider_specific_policies() {
    ...
    assert!(openai_delay.unwrap() > anthropic_delay.unwrap(), "OpenAI should have longer rate limit delays");
    ...
    assert!(ollama_delay.unwrap() < openai_transport_delay.unwrap(), "Ollama should retry faster for transport errors");
}
...
async fn test_retry_backoff_progression() {
    ...
    assert!(delay10 <= Duration::from_millis(policy.max_delay_ms), "Delay should be capped at max");
}
...
async fn test_jitter_prevents_thundering_herd() {
    ...
    for delay in &delays {
        assert!(*delay >= Duration::from_millis(1500), "Delay should be at least base * multiplier");
        assert!(*delay <= Duration::from_millis(2500), "Delay should not exceed reasonable jitter range");
    }
}
